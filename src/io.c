                 ...
                 bool                 verbose)
{
  ...
  if (padding > 0) {
    block_amount = (in_file_size / AES_BLOCK_SIZE) + 1;  // +1 block size
  }
  else if (padding == 0) {
    block_amount = in_file_size / AES_BLOCK_SIZE;
  }

  unsigned char block[AES_BLOCK_SIZE];
  unsigned char ciphered_block[AES_BLOCK_SIZE + AES_BLOCK_SIZE];  // ciphered block could be padded by EVP up to 32 bytes
  ...
  for (unsigned int i = 0; i < block_amount; i++) {
    if (verbose) printf("block is %d \n", i);

    read_len = fread(block, sizeof(unsigned char), AES_BLOCK_SIZE, input_file);

    if (verbose) printf("read len : %ld\n", read_len);
    ...
    ciphered_len = aes_encrypt(ctx, block, read_len, ciphered_block,
                               (bool)(i == block_amount - 1));  // finilize in case last block

    if (verbose) printf("encrypted len %ld\n", ciphered_len);
    ...
    if (verbose) printf("written to file : %ld\n", write_len);
    fseek(output_file, 0L, SEEK_END);

    memset(block, 0, AES_BLOCK_SIZE);
    memset(ciphered_block, 0, AES_BLOCK_SIZE);
  }
  ...
  write_len = fwrite((unsigned char *)&header, sizeof(unsigned char), sizeof(struct header), output_file);
  if (write_len != sizeof(struct header)) {
    printf("Bad fwrite() \n");
    fclose(input_file);
    fclose(output_file);
    return EXIT_FAILURE;
                 ...
                 bool                 verbose)
{
  ...
  unsigned int block_amount = in_file_size / AES_BLOCK_SIZE;

  unsigned char ciphered_block[AES_BLOCK_SIZE + AES_BLOCK_SIZE];
  unsigned char block[AES_BLOCK_SIZE];

  if (verbose) printf("block_amount: %d\n", block_amount);

  for (unsigned int i = 0; i < block_amount; i++) {
    if (verbose) printf("block: %d \n", i);

    read_len = fread(ciphered_block, sizeof(unsigned char), AES_BLOCK_SIZE, input_file);
    ...
    if (verbose) printf("plaintext len : %ld\n", plaintext_len);
    ...
    if (verbose) printf("write_len : %ld\n", write_len);
  }
